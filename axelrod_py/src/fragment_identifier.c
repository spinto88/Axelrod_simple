...
void fragment_identifier(axl_network mysys, axl_node *node_info)
{
	...
	for(i = 0; i < n; i++)
	{
		...
		while(k < n)
		{
			...
			if(((is_neighbor(node_info[node1], node2)) == 1) && (is_same_state(mysys.agent[node1], mysys.agent[node2]) == 1))
