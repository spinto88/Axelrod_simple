...
void fragment_identifier(axl_network mysys, axl_node *node_info, int clustering_radio, int type_serch)
{
	...
	for(i = 0; i < n; i++)
	{
		...
		while(k < n)
		{
			...
			if(((is_neighbor(node_info[node1], node2)) == 1) && (is_same_state(mysys, node1, node2, clustering_radio,type_serch) == 1))
